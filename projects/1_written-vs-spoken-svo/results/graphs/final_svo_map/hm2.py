...
SAMPLE_SIZES_NOUN = {
    ...
    "French Written": 2051,
    "French Spoken": 53,
    "Norwegian Written (nynorsk)": 2045,
    "Norwegian Spoken (nynorsk)": 33,
    "Slovenian Written": 1969,
    "Slovenian Spoken": 168,
    "Spanish Written": 1874,
    "Spanish Spoken": 12,
}

SAMPLE_SIZES_ALL = {
    "English Written": 2930,
    "English Spoken": 2368,
    "French Written": 7718,
    "French Spoken": 1071,
    "Norwegian Written (nynorsk)": 7669,
    "Norwegian Spoken (nynorsk)": 1498,
    "Slovenian Written": 3908,
    "Slovenian Spoken": 835,
    "Spanish Written": 3985,
    "Spanish Spoken": 121,
}
...
def draw_heatmap(ax, heatmap_data, col_positions, sample_sizes=None):
...
def create_custom_heatmap(df, title, filename, sample_sizes=None):
    ...
    draw_heatmap(ax, heatmap_data, col_positions, sample_sizes)
    ...
    # Create labels with sample sizes if provided
    if sample_sizes is not None:
        # First, set the base labels
        ax.set_xticklabels(BASE_LABELS, fontsize=10, rotation=0)
        
        # Then add sample sizes as separate text elements, positioned above the x-axis label
        for i, col_name in enumerate(COLUMN_ORDER):
            if col_name in sample_sizes:
                xpos = col_positions[i] + 0.5
                # Position just above where the x-axis label will be (which is at labelpad=15 below)
                ax.text(xpos, heatmap_data.shape[0] + 1.0, f"(n={sample_sizes[col_name]})", 
    ...
    ax.set_xlabel("Language (W = Written, S = Spoken)", fontsize=11, labelpad=25)
...
for df, title, path, description, sample_sizes in jobs:
    create_custom_heatmap(df, title, path, sample_sizes)
    print(f"Saved: {path}")
